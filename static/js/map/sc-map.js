...
class SCMap {
    ...
    _stateTipHtml(p) {
        ...
        let html = `<div class="tooltip-title">${p.name}</div><div class="tooltip-row"><span class="tooltip-label">Macro Região:</span> <span class="tooltip-value">${p.macro_region}</span></div><div class="tooltip-row"><span class="tooltip-label">População:</span> <span class="tooltip-value">${fmt.number(p.population)}</span></div><div class="tooltip-row"><span class="tooltip-label">Contatos:</span> <span class="tooltip-value">${p.total_contacts || 0}</span></div><div class="tooltip-row"><span class="tooltip-label">Votos LS 2022:</span> <span class="tooltip-value">${fmt.number(p.total_votes_2022)}</span></div>`;
    ...
    _cityTipHtml(p) {
        let html = `<div class="tooltip-title">${p.name}</div><div class="tooltip-row"><span class="tooltip-label">População:</span> <span class="tooltip-value">${fmt.number(p.population)}</span></div><div class="tooltip-row"><span class="tooltip-label">Votos LS 2022:</span> <span class="tooltip-value">${fmt.number(p.votes_2022)}</span></div><div class="tooltip-row"><span class="tooltip-label">Contatos:</span> <span class="tooltip-value">${p.total_contacts || 0}</span></div>`;
        if (this.heatmapEnabled) {
    ...
    _renderTransferCityMap() {
        ...
        // Limpar tudo e redesenhar com polígonos de cidade
        ...
        this.g.selectAll('path.transfer-city')
            ...
            .on('click', (event, d) => {
                self._hideTip();
                window.location.href = `/cidade/${d.properties.slug}/?mapa=${self.mapMode}`;
            });
    ...
    async setItineraries(enabled, showCompleted = false) {
        ...
        if (enabled) {
            ...
            this.elections2022Enabled = false;
            try {
                this._itinerariesData = await API.itineraries.mapData(showCompleted);
            } catch (e) {
    ...
    async setDemands(enabled) {
        ...
        if (enabled) {
            try {
                this._demandsData = {};
                const data = await API.tasks.mapStatus();
                for (const d of data) {
    ...
    _plNetworkCityTipHtml(p) {
        ...
        html += `<div class="tooltip-row"><span class="tooltip-label">Contatos</span> <span class="tooltip-value">${city.contacts}</span></div>`;
        return html;
    }
    ...
    async zoomToRegion(slug) {
        try {
            ...
            if (!geojson.features || geojson.features.length === 0) {
                window.location.href = `/regiao/${slug}/?mapa=${this.mapMode}`;
                return;
            }
            ...
            this.g.selectAll('path.city')
                ...
                .on('click', (event, d) => {
                    ...
                    else window.location.href = `/cidade/${d.properties.slug}/?mapa=${this.mapMode}`;
                });
